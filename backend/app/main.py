...
# ---------- 第一梯队：北向资金 / 板块轮动 / 条件选股 / 融资融券 ----------

from .data import north_flow, sector_flow, stock_screener, margin_data as margin_mod


@app.get("/api/north-flow/overview")
def north_flow_overview_api() -> dict[str, Any]:
    """北向资金总览：近30天净流入趋势。"""
    return north_flow.get_north_flow_overview()


@app.get("/api/north-flow/stock/{symbol}")
...
@app.get("/api/north-flow/top-stocks")
